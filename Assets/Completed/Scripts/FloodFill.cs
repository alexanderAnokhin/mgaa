...
namespace Completed
{
	...
	//Transform map to another view
		...
		public  Cell[,] AddCell(Cell[,] cell,Vector3 position, string tag)
		{
			...
			/*if (tag == "Wall")
			{
				cell[(int) Math.Round(position.x),(int) Math.Round(position.y)].type = "w";
			}
			else
			{
				if(tag == "Enemy")
				{
					cell[(int) Math.Round(position.x),(int) Math.Round(position.y)].type = "e";
				}
				else
				{
					cell[(int) Math.Round(position.x),(int) Math.Round(position.y)].type = "n";
				}
			}*/
	                ...
	                //{
	                	if (current.x+1 < cells.GetLength(0))
	                	{
	                		if (checkAvailability(cells[current.x+1,current.y]))
	                		{
	                			...
	                			//Debug.Log("x - "+checker+", y - "+current.y);
	                		}
	                	}
	                	if (current.y+1 < cells.GetLength(1))
	                	{
	                		if (checkAvailability(cells[current.x,current.y+1]))
	                		{
	                			...
	                			//Debug.Log("y - "+checker+"; x - "+current.x);
	                		}
	                	}
	                	if (current.x-1 >= 0)
	                	{
	                		if (checkAvailability(cells[current.x-1,current.y]))
	                		{
	                			cells[current.x-1,current.y].type = "p";
	                			q.Enqueue(cells[current.x-1,current.y]);
	                			//Debug.Log("y - "+current.y+"; x - "+checker);
	                		}
	                	}
	                	if (current.y-1 >= 0)
	                	{
	                		if (checkAvailability(cells[current.x,current.y-1]))
	                		{
	                			cells[current.x,current.y-1].type = "p";
	                			q.Enqueue(cells[current.x,current.y-1]);
	                			//Debug.Log("y - "+checker+"; x - "+current.y);
	                		}
	                	}
